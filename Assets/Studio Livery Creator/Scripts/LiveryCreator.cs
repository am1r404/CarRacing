...
public class LiveryCreator : MonoBehaviour
{
    ...
    IEnumerator SaveTextureToFile(Texture2D savedTexture)
    {
        if (savedTexture != null)
        {
            ...
            string fullPath = System.IO.Directory.GetCurrentDirectory() + "/Studio Livery Creator/MyLiveries/" + dateTime;
            string fileName = dateTime;
            if (!System.IO.Directory.Exists(fullPath))
                System.IO.Directory.CreateDirectory(fullPath);
            var bytes = savedTexture.EncodeToPNG();
            System.IO.File.WriteAllBytes(Application.dataPath + "/Studio Livery Creator/MyLiveries/" + fileName, bytes);
