    ...
    public static class ES3Reflection
    {
        ...
        public static Type GetType(string typeString)
        {
            switch (typeString)
            {
                ...
                default:
                    return GetTypeInternal(typeString);
            }
        }

        private static Type GetTypeInternal(string typeString)
        {
            Type returnType = null;
            try
            {
                returnType = Type.GetType(typeString, true);
            }
            catch
            {
                Debug.LogError($"{typeString} type not fount");
                var newString = $"Legacy.{typeString}";
                returnType = Type.GetType(newString);
            }
            return returnType;
        }
